...
安装脚本会自动：
1. 检测系统和 CPU 架构
2. 检测已安装的 Web 服务（Nginx/Apache/IIS）
3. 下载对应版本的部署工具

**手动安装:**

从 [Releases](https://github.com/zhuxbo/cert-deploy/releases) 下载对应平台的二进制文件：

```bash
# Linux/macOS
gunzip cert-deploy-nginx-linux-amd64.gz
chmod +x cert-deploy-nginx-linux-amd64
sudo mv cert-deploy-nginx-linux-amd64 /usr/local/bin/cert-deploy-nginx
```
