...
func TestRunCommand(t *testing.T) {
	tests := []struct {
		...
		{"空命令", "", true},
		{"只有空格", "   ", true},
		{"包含分号", "echo hello; echo world", true},
		{"包含管道", "echo hello | cat", true},
		{"包含 &", "echo hello &", true},
		{"包含 $", "echo $HOME", true},
		{"包含反引号", "echo `whoami`", true},
		{"包含反斜杠", "echo hello\\nworld", true},
		{"正常 echo 命令", "echo hello", false},
		{"不存在的命令", "nonexistent_command_xyz", true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("RunCommand(%q) error = %v, wantErr %v", tt.command, err, tt.wantErr)
			}
		})
	}
}

// TestRunCommand_Success 测试成功执行命令
func TestRunCommand_Success(t *testing.T) {
	// 测试 true 命令（总是成功）
	if err := RunCommand("true"); err != nil {
		t.Errorf("RunCommand(true) should succeed, got error: %v", err)
	}
}

// TestRunCommand_Failure 测试失败的命令
