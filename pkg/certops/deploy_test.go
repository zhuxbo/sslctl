...
func TestDeployToBinding_UnsupportedServerType(t *testing.T) {
	...
	// 测试不支持的服务器类型
	err = svc.deployToBinding(nil, binding, certData, testCert.KeyPEM)
	if err == nil {
...
func TestDeployToBinding_InvalidCert(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := svc.deployToBinding(nil, binding, certData, "fake-key")
...
func TestDeployToBinding_MismatchedKeyPair(t *testing.T) {
	...
	err = svc.deployToBinding(nil, binding, certData, mismatchedPair.WrongKey)
	if err == nil {
...
func TestDeployToBinding_WithIntermediateCert(t *testing.T) {
	...
	err = svc.deployToBinding(nil, binding, certData, chain.LeafKeyPEM)
	if err != nil {
