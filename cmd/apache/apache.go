...
func checkAndDeploy(ctx context.Context, cfgManager *config.Manager, log *logger.Logger) {
	...
	for _, site := range sites {
		...
		// 根据续签模式选择处理逻辑
		if site.IsLocalKeyMode() {
			if err := renewLocalKeyMode(ctx, cfgManager, site, certPath, keyPath, chainPath, webroot, log); err != nil {
				log.Error("站点 %s 本地私钥模式续签失败: %v", site.SiteName, err)
			}
		} else {
			if err := renewPullMode(ctx, cfgManager, site, certPath, keyPath, chainPath, webroot, log); err != nil {
				log.Error("站点 %s 拉取模式续签失败: %v", site.SiteName, err)
			}
		}
	}
...
func renewLocalKeyMode(ctx context.Context, cfgManager *config.Manager, site *config.SiteConfig, certPath, keyPath, chainPath, webroot string, log *logger.Logger) error {
	...
	case issuer.ActionDeployed:
		// 部署证书
		site.Metadata.OrderID = result.OrderID
		return deployWithCertData(cfgManager, site, certPath, keyPath, chainPath, result.CertData, result.PrivateKey, log)
...
//   - 失败/status != active: 跳过
//   - 保存 order_id 并部署
func renewPullMode(ctx context.Context, cfgManager *config.Manager, site *config.SiteConfig, certPath, keyPath, chainPath, webroot string, log *logger.Logger) error {
	...
	// 部署证书
	return deployWithCertData(cfgManager, site, certPath, keyPath, chainPath, certData, certData.PrivateKey, log)
}
