...
func TestResolveReleaseURL_FromConfig_Trims(t *testing.T) {
	cm, err := config.NewConfigManagerWithDir(t.TempDir())
	if err != nil {
		t.Fatalf("NewConfigManagerWithDir error: %v", err)
	}
	cfg := &config.Config{ReleaseURL: "https://localhost/"}
...
func TestResolveReleaseURL_EmptyNonTerminal(t *testing.T) {
	cm, err := config.NewConfigManagerWithDir(t.TempDir())
	if err != nil {
		t.Fatalf("NewConfigManagerWithDir error: %v", err)
	}
	cfg := &config.Config{ReleaseURL: ""}
	...
	if err == nil {
		t.Fatal("expected error for empty release_url in non-terminal")
	}
	if !strings.Contains(err.Error(), "未配置升级地址") {
